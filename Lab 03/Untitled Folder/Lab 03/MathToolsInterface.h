...
struct NegativeDiscriminant: public std::exception{
	const char *what() const throw(){
		return"NegativeDiscriminant";
	}
};

struct CannotDetermineRoomDimensions: public std::exception{
	const char *what() const throw(){
		return"CannotDetermineRoomDimensions";
	}
};
...
public:
	 ...
	 */
	virtual int intersectLineUnitCircle(double d, double e, double f, double xy[2][1])
