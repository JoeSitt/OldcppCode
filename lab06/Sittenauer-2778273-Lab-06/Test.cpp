...
bool Test::testWalker(std::string testFileName, const int* const* correctArray, int correctRows, int correctCols)
{
	bool isPassed = false;
	...
	try
	{
		...
		MazeWalker walker(mr.getMaze(), mr.getStartRow(), mr.getStartCol(),
						 mr.getRows(), mr.getCols());

		walker.walkMaze(mr.getStartRow(), mr.getStartCol());
...
bool Test::test_mazeWalker01()
{
	...
	isPassed = testWalker(testFileName, correctArray, correctRows, correctCols);
